...
# alternate method for extracting predictions:
LWfunction <- function(a = exp(coef(LxW.lm)[1]), b = coef(LxW.lm)[2], Length) {
  predW <- a*Length^b
}

Pred2<- LWfunction(Length = c(21:100))

tibble(L,W,Pred2) %>% ggplot(aes(L,W)) + geom_point() + geom_line(aes(L,Pred2))
```

## Length-at-age relationships

Even though growth is obviously a very important biological relationship, and body growth is an important process, it is not incorporated directly into age-based modeling, and therefore never estimated. Growth processes are implicitly included in the creation of the ALK. Therefore, by plotting and fitting for example, a Von Bertalanffy growth curve, we can gain a better understanding of our data and biological processes, even if it is not directly incorporated into the modeling framework. This kind of exploratory analysis can be extremely important, for example, when determining whether different ALKs are necessary to represent different segments or sub-components of the population or fishing activity (see previous section).

In this example, let's consider two growth curves that look very different. They are intended to reflect growth similar to haddock versus golden redfish.
