...
Let's try to repeat the same figure but with variability implemented:
...
tibble(EquilCatch, Efforts) %>% ggplot(aes(Efforts, EquilCatch)) + geom_point() + ylab("Equilibrium Catch")
...
for(j in 1:length(Efforts)){
	results<-AgeStructModel(Rec, Effort = Efforts[j])
	NumbersResults[[j]]<-results[[1]][99,]
	CatchResults<-apply(results[[2]], 1, sum, na.rm=T)
	EquilCatch[j]<-CatchResults[99]
	}

NumbersResults %>% 
  bind_rows(NumberResults, .id = 'id') %>%
  mutate(effort = Efforts) %>% 
  gather(value = 'numbers', key = 'effort') %>% 
  filter(effort %in% c(0.1, 0.5, 1, 1.5, 1.9)) %>% 
  group_by(effort) %>% 
  ggplot(aes(numbers)) + geom_col() + ylab("Equilibrium Catch") + facet_wrap(~effort)
