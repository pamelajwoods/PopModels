...
AgeStructModel<-function(Rec, AA, Effort = rep(1, length(Rec)), q=1, Sel = c(0,0,rep(1,AA-1))){
	YY <- 1:length(Rec)
...
#run loop through all ages and years. Each of the 3 types of equations are used.
	...
	for(i in 2:YY){
	 ...
	 #because all ages are not available until after the 11th year, we begin calculating catch then
	  if(YY> 11){CatchNM[i-1,]<-NumbersM[i-1,]*FF[i-1,]/ZZ[i-1,]*(1-exp(-ZZ[i-1,]))}

	#fishes are born
	  NumbersM[i,1] <- Rec[i] - CatchNM[i-1,1]
	for(j in 2:(AA)){

	  	NumbersM[i, j]<-NumbersM[i-1, j-1]*exp(-ZZ)  - CatchNM[i-1,j]
	}
	  
	  NumbersM[i, AA+1]<-NumbersM[i-1, AA]*exp(-ZZ) + NumbersM[i, AA+1]*exp(-ZZ) - CatchNM[i-1,AA+1]
...
NumbersResults<-AgeStructModel(0.5)[[1]]
CatchResults<-AgeStructModel(0.5)[[2]]
