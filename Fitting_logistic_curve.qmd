...
Remember this equation from the [Simple Population Models](Simple_population_models.qmd) section?
...
# Here we are comparing the fit based on trying two different optimization 
# algorithms. Note the starting values are the same, but the 'L-BFGS-B'
# method requires corresponding upper and lower boundary limits to be
# inputted (OptPars1). The OptPars2 version uses a default "Port algorithm".
...
Do any of the values in the resulting OptPars1 function hit a bound? That means it is not well estimated. Standard practice is to change the bound to try to widen the search space and let the algorithm find the best value. But if that doesn't work, then perhaps it is not possible to estimate this parameter. Too many 'right' answers are avaible - a large space of different values for this parameter lead to the same answer.

However, when judging whether we should optimize a loose parameter versus fix its value, it is also important to judge the model's resulting realism. Which of the above versions have more biological realism for elephants?
