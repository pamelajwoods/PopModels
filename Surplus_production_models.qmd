...
par(mfrow = c(1,3))
p1 <- tibble(PP,DD) %>% ggplot(aes(PP,DD)) + geom_point() + xlab("Population N") + ylab("Increase in N  from t to t+1 (dN / dt)")
...
p2 <- tibble(PP,DD) %>% ggplot(aes(PP,DD)) + geom_point() + xlab("Population N") + ylab("Increase in N  from t to t+1 (dN / dt)")

#Now graph the population over time alongside the above:
p3 <- tibble(PP,TT) %>% ggplot(aes(TT,PP)) + geom_point() + xlab("Time") + ylab("Population N")
...
-   At what population biomass level is population growth the greatest? (Remember to distinguish between *growth rate*, which is a rate, and growth amount, *catch*, which is a single unit of removal in kg or tonnes in a time step)
