...
-   the relationship between population size and growth (biomass -\> growth).

-   how growth translates controls total fish removals in terms of tonnes or kg (growth -\> catch).
...
tibble(YY, eff, Cost) %>% ggplot(aes(eff, YY)) + geom_point() + ylab("Yield") + xlab("Effort") + geom_point(aes(eff, Cost), shape = 2) + 
  geom_vline(aes(xintercept = eff[which.max(YY)])) +
  geom_vline(aes(xintercept = eff[which.max(YY - Cost)] ), color = 'red')
...
[Here](Pella_Tomlinson_Ex_template.Rmd) are sections that can be put into an Rmarkdown file as a template to get you started.
...
[Here](Pella_Tomlinson_Ex.Rmd) is a final version.
